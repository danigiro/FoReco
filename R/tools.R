  ...
  if (missing(agg_mat) && missing(cons_mat)) {
    ...
    if (flag) {
      ...
      if (sparse) {
        ...
        return(list(dim = c("n" = n), cons_mat = as.matrix(cons_mat)))
      }
    }
  }
...
tetools <- function(agg_order, fh = 1, tew = "sum", sparse = TRUE) {
  ...
  if (!all(kset %in% rev(all_factors(m)))) {
    cli_abort(
      "({paste(kset, collapse = ', ')}) is not a subset
              of ({paste(rev(all_factors(m)), collapse = ', ')}).",
      call = NULL
    )
  }
  ...
  if (is.character(tew[[1]][1])) {
    ...
    if (length(tew) != (p - 1)) {
      cli_abort(
        "{.arg weights} is not a list with {p-1} elements.",
        call = NULL
      )
    }
    ...
    if (any(sapply(tew, length) != kset[-p])) {
      cli_abort(
        "The {p-1} elements of {.arg weights} must be a vector of length {kset[-p]}, respectively.",
        call = NULL
      )
    }
    weights <- tew
  ...
  agg_mat <- do.call(
    ...
    Map(
      ...
      lapply(m / kset[-p], function(x) .sparseDiagonal(x * fh)),
...
cttools <- function(
  ...
  Cup <- cbind(
    Matrix(0, fh * NROW(Ccs) * m, n * fh * ks),
    kronecker(.sparseDiagonal(fh * m), Ccs)
  ) %*%
    P
  ...
  if (!is.null(Acs)) {
    agg_mat <- rbind(
      kronecker(Acs, Ste),
      kronecker(.sparseDiagonal(NCOL(Acs)), Ate)
    )
    strc_mat <- kronecker(Scs, Ste)
    return(sparse2dense(
      list(
        dim = c(csobj$dim, teobj$dim),
        set = teobj$set,
        agg_mat = agg_mat,
        strc_mat = strc_mat,
        cons_mat = cons_mat
    ...
    return(sparse2dense(
      list(dim = c(csobj$dim, teobj$dim), set = teobj$set, cons_mat = cons_mat),
      sparse = sparse
    ))
  }
}
