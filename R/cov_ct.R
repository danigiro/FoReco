...
#' errors covariance matrix using different reconciliation methods (Di Fonzo
...
ctcov.ols <- function(comb = "ols", ..., n = NULL, agg_order = NULL) {
  if (is.null(n) || is.null(agg_order)) {
    cli_abort(
      "Argument {.arg n} and/or {.arg agg_order} are NULL.",
      call = NULL
    )
  }
    ...
    if (!is.null(tmp$res)) {
      cli_warn(
        c(
          ...
          "i" = "When {.arg res} is provided, {.arg comb = 'bdshr'} is suggested."
        ),
        call = NULL
      )
    }
...
ctcov.str <- function(
  ...
  if (is.null(strc_mat)) {
    if (is.null(agg_mat) || is.null(agg_order)) {
      cli_abort(
        "Argument {.arg agg_mat} and/or {.arg agg_order} are NULL.",
        call = NULL
      )
    }
...
ctcov.wlsv <- function(
  ...
  m <- max(kset)
  var_freq <- apply(res, 1, function(z) {
    sapply(kset, function(x) {
...
ctcov.acov <- function(
  ...
  mat1 <- bdiag(rep(
    lapply((m / kset), function(x) matrix(1, nrow = x, ncol = x)),
    n
  ))
...
ctcov.Ssam <- function(
  ...
  mat1 <- bdiag(replicate(
    n,
    matrix(1, nrow = sum(max(kset) / kset), ncol = sum(max(kset) / kset)),
    simplify = FALSE
  ))
  sample_estim(res_mat, mse = mse) * mat1
}
...
ctcov.Sshr <- function(
  ...
  shrink <- lapply(1:n, function(x) {
    shrink_fun(
      res_mat[, rep(1:n, each = sum(max(kset) / kset)) == x],
      mse = mse
    )
  })
  bdiag(shrink)
...
ctcov.bdsam <- function(
  ...
  N <- NCOL(res) / sum(max(kset) / kset)
  Wlist <- lapply(kset, function(x) {
    sample_estim(t(res[, rep(kset, N * (m / kset)) == x]), mse = mse)
  })
  W <- rep(Wlist, (m / kset))
  P <- commat(n, sum(max(kset) / kset))
  P %*% bdiag(W) %*% t(P)
