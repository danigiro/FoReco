...
iterec <- function(
  ...
  if (missing(cslist) | missing(telist)) {
    cli_abort(
      "Argument {.arg cslist} and/or {.arg telist} are missing,
              with no default.",
      call = NULL
    )
  }
  ...
  if (any(names(telist) == "agg_order")) {
    ...
    te_const_mat <- tetools(
      agg_order = telist$agg_order,
      fh = h,
      tew = telist$tew
    )$cons_mat
  } else {
    cli_abort(
      "Argument {.arg agg_order} is missing in {.arg telist},
              with no default.",
      call = NULL
    )
  }
  ...
  if (any(names(cslist) %in% c("agg_mat", "cons_mat"))) {
    ...
    cli_abort(
      "Argument {.arg agg_mat} (or {.arg cons_mat}) is missing in {.arg cslist},
              with no default.",
      call = NULL
    )
  }
  ...
  if (!any(names(telist) == "comb") & verbose) {
    cli_alert_warning(c(
      "Argument {.arg comb} is missing in {.arg telist}. ",
  ...
  if (!any(names(cslist) == "comb") & verbose) {
    cli_alert_warning(c(
      "Argument {.arg comb} is missing in {.arg cslist}. ",
      "Default option: {.strong {.code ols}}."
    ))
    cslist$comb <- "ols"
  }
  ...
  names_label <- c("cs" = "Cross-sectional", "te" = "Temporal")
  flag <- -1
  tmp <- base
  dmat <- array(
    NA,
  ...
  dmat[,, 1, 2] <- dmat[,, 1, 1] <- discrepancy(
    base,
    cs_const_mat = cs_const_mat,
  ...
  if (verbose) {
    cli_rule(
      "{.strong Iterative heuristic cross-temporal forecast reconciliation}"
    )
    cli_text(
      "{.emph Legend: i\u00a0=\u00a0iteration; s\u00a0=\u00a0step. Norm\u00a0=\u00a0\"{norm}\".}"
    )
    cli_text("\n")
    ...
    c1r1 <- gsub(
      " ",
      "\u00a0",
      format("i.s", width = nchar(itmax) + 2, justify = "right")
    )
    c2r1 <- gsub(
      " ",
      "\u00a0",
      format(names_label[rorder[1]], width = pwidth, justify = "right")
    )
    c3r1 <- gsub(
      " ",
      "\u00a0",
      format(names_label[rorder[2]], width = pwidth, justify = "right")
    )
  ...
  for (i in 1:itmax) {
    ...
    if (verbose) {
      text_num <- paste0(
        formatC(i + 0.1, width = nchar(itmax) + 2, format = "f", digits = 1),
        " | ",
    ...
    if (verbose) {
      text_num <- paste0(
        formatC(i + 0.2, width = nchar(itmax) + 2, format = "f", digits = 1),
        " | ",
...
check_results <- function(flag, dmat, i, itmax, norm, verbose) {
  if (flag == 0) {
    ...
    check_sorted <- all(apply(dmat[, norm, , ], c(1, 3), function(x) {
      !is.unsorted(rev(na.omit(x[-1])))
    }))
    if (!check_sorted) {
    ...
    if (verbose) {
      cli_text("\n")
      cli_alert_danger(
        "Convergence NOT achieved. Maximum number of iterations reached ({itmax})."
      )
    }
    return(flag)
  }
...
discrepancy <- function(mat, cs_const_mat, te_const_mat) {
  dmat <- matrix(
    ...
    nrow = 2,
    ncol = 3,
    dimnames = list(c("cs", "te"), c("inf", "one", "two"))
  )
  dmat[, 1] <- c(
