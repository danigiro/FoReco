...
if (require(testthat)) {
  ...
  set.seed(123)
  base <- rbind(
    rnorm(7, rep(c(40, 20, 10), c(1, 2, 4))),
  ...
  test_that("Cross-temporal tools", {
    expect_equal(
      cttools(agg_mat = A, agg_order = agg_order, sparse = FALSE)[c(
        "dim",
        "set"
      )],
      list(
        dim = c(n = 7, na = 3, nb = 4, m = 4, p = 3, ks = 3, kt = 7),
        set = c(4, 2, 1)
      )
    )
  })
  ...
  test_that("Covariance", {
    for (i in c(
      ...
      if (i %in% c("ols", "str", "csstr", "testr")) {
        expect_no_error(ctrec(
          base = base,
          agg_mat = A,
          agg_order = agg_order,
          comb = i
