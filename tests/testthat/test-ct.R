...
if(require(testthat)){
  ...
  test_that("Bounds", {
    tmp <- set_bounds(n = c(4:7), k = 1, h = 1, lb = 3)
    r1 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "strc", bounds = tmp)
    r2 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "proj", bounds = tmp)
    expect_equal(r1[, "k-1 h-1"], c(12, 6, 6, 3, 3, 3, 3), ignore_attr = TRUE)
    expect_equal(r1, r2, ignore_attr = TRUE)

    tmp <- set_bounds(n = c(4:7), k = 1, h = 1, lb = 3, approach = "sftb")
    r3 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "strc", bounds = tmp)
    expect_equal(r3[, "k-1 h-1"], r1[, "k-1 h-1"], ignore_attr = TRUE)

    tmp <- set_bounds(n = 8, k = 1, h = 1, lb = 3)
    expect_warning(ctrec(base = base, agg_mat = A, agg_order = agg_order,
                         comb = comb, res = res, bounds = tmp))
  })

  test_that("Cross-temporal reconciliation with agg_order subset", {
    r1 <- ctrec(base = base[, -c(2:3)], agg_mat = A, agg_order = c(4, 1),
                comb = "wlsv", res = res[, -c(11:30)])
    r2 <- iterec(base = base[, -c(2:3)],
                 cslist = list(agg_mat = A, comb = "wls"),
                 telist = list(agg_order = c(4, 1), comb = "wlsv"),
                 res = res[, -c(11:30)], verbose = FALSE, tol = 1e-8)


    r3 <- ctrec(base = base[, -c(2:3)], agg_mat = A, agg_order = c(4, 1),
                comb = "ols", res = res[, -c(11:30)])
    r4 <- iterec(base = base[, -c(2:3)],
                 cslist = list(agg_mat = A, comb = "ols"),
                 telist = list(agg_order = c(4, 1), comb = "ols"),
                 res = res[, -c(11:30)], verbose = FALSE)

    r5 <- ctrec(base = base[, -c(2:3)], agg_mat = A, agg_order = c(4, 1),
                comb = "str", res = res[, -c(11:30)])
    r6 <- iterec(base = base[, -c(2:3)],
                 cslist = list(agg_mat = A, comb = "str"),
                 telist = list(agg_order = c(4, 1), comb = "str"),
                 res = res[, -c(11:30)], verbose = FALSE)
    r6k <- tcsrec(base = base[, -c(2:3)],
                  cslist = list(agg_mat = A, comb = "str"),
                  telist = list(agg_order = c(4, 1), comb = "str"),
                  res = res[, -c(11:30)])

    r7 <- ctrec(base = base[, -c(2:3)], agg_mat = A, agg_order = c(4, 1),
                comb = "csstr", res = res[, -c(11:30)])
    r8 <- iterec(base = base[, -c(2:3)],
                 cslist = list(agg_mat = A, comb = "str"),
                 telist = list(agg_order = c(4, 1), comb = "ols"),
                 res = res[, -c(11:30)], verbose = FALSE)
    r8k <- tcsrec(base = base[, -c(2:3)],
                  cslist = list(agg_mat = A, comb = "str"),
                  telist = list(agg_order = c(4, 1), comb = "ols"),
                  res = res[, -c(11:30)])

    r9 <- ctrec(base = base[, -c(2:3)], agg_mat = A, agg_order = c(4, 1),
                comb = "testr", res = res[, -c(11:30)])
    r10 <- iterec(base = base[, -c(2:3)],
                  cslist = list(agg_mat = A, comb = "ols"),
                  telist = list(agg_order = c(4, 1), comb = "str"),
                  res = res[, -c(11:30)], verbose = FALSE)
    r10k <- tcsrec(base = base[, -c(2:3)],
                   cslist = list(agg_mat = A, comb = "ols"),
                   telist = list(agg_order = c(4, 1), comb = "str"),
                   res = res[, -c(11:30)])
  ...
  test_that("Optimal nonegative cross-temporal reconciliation", {
    r1 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
    ...
    rf <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res)
    rbu <- ctbu(rf[-c(1:3),-c(1:3)], agg_mat = A,
                agg_order = agg_order, sntz = TRUE)
  ...
  test_that("Optimal immutable cross-temporal reconciliation", {
    r1 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "strc", immutable = c(1, 4, 1))
    r2 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "proj", immutable = c(1, 4, 1))
    r3 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "strc_osqp", immutable = c(1, 4, 1))
    r4 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "proj_osqp", immutable = c(1, 4, 1))
    r5 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "strc_osqp", immutable = c(1, 4, 1),
                nn = "osqp")
    r6 <- ctrec(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res, approach = "proj_osqp", immutable = c(1, 4, 1),
                nn = "osqp")
  ...
  test_that("ctlcc and BU", {
    r0 <- ctlcc(base = base, agg_mat = A, agg_order = agg_order, comb = comb,
                res = res)
    r1 <- ctlcc(base = base, agg_mat = A, agg_order = agg_order,
                nodes = c(1, 2), comb = comb, res = res)
    r2 <- ctbu(base[-c(1:3),-c(1:3)], agg_mat = A, agg_order = agg_order)
    r3 <- ctbu(base[-c(1:3),-c(1:3)], agg_mat = A, agg_order = agg_order,
               round = TRUE)

    expect_error(ctbu(base, agg_order = agg_order, agg_mat = A))
    expect_error(ctbu(base, agg_order = agg_order))
    expect_error(ctbu(agg_order = agg_order))
    expect_error(ctbu(base[-c(1:3),-c(1:3)], agg_mat = A))
  ...
  test_that("Top-down and Middle-out", {
    ...
    r1 <- cttd(base = topf, agg_mat = A, agg_order = agg_order,
               weights = fix_weights)

    h_weights <- cbind(fix_weights, fix_weights)
    r2 <- cttd(base = topf, agg_mat = A, agg_order = agg_order,
               weights = h_weights)

    # Normalization check
    r3 <- cttd(base = topf, agg_mat = A, agg_order = agg_order,
               weights = fix_weights/sum(fix_weights))

    # Middle-out
    r4 <- ctmo(base = topf, agg_mat = A, agg_order = agg_order,
               weights = fix_weights)
    r5 <- ctmo(base = topf, agg_mat = A, agg_order = agg_order,
               weights = h_weights)
  ...
  test_that("Cross-temporal tools", {
    M <- ctprojmat(cons_mat = Ccs, agg_order = agg_order, comb = comb,
                   res = res)
    G <- ctprojmat(agg_mat = A, agg_order = agg_order, comb = comb, res = res,
                   mat = "G")
  ...
  test_that("Covariance", {
    for(i in c("acov", "bdsam", "bdshr", "bsam", "bshr", "csstr", "hbsam",
               "hbshr", "hsam", "hshr", "ols", "sam", "shr", "Ssam", "Sshr",
               "str", "testr", "wlsh", "wlsv")){
      expect_no_error(ctrec(base = base, agg_mat = A, agg_order = agg_order,
                            comb = i, res = res))
    }
  })
