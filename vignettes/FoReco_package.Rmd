...
The first 14 years of each simulated series are used as training set, and the last year as test set. The forecasts are obtained using the `auto.arima` function of the `forecast` package (Hyndman et al., 2020). The following plots show the actual values and the forecasts for the test year at any temporal aggregation level. 
```{r eval = FALSE}
