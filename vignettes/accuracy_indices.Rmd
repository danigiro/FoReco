...
$$
\begin{array}{rcl}
MAE_{i,j}^{[k],h} & = & \displaystyle\frac{1}{q}\displaystyle\sum_{t=1}^{q} \left(\hat{e}_{i,j}^{[k],h}\right)^2 \\
...
\quad i=1,\ldots,n, \quad  j=0,\ldots, J, \quad k \in {\cal K}, \quad h=1, \ldots, M_k,
$$
...
$$
\label{AvgRelAikh1h2}
\text{AvgRelA}_{i,j}^{[k],h_1:h_2} = \left(\prod_{h=h_1}^{h_2} r_{i,j}^{[k],h}\right)^{\frac{1}{h_2 - h_1 + 1}},
\; i=1,\ldots,n , \; j=0,\ldots, J, \; k \in {\cal K}.
$$
...
\left(\displaystyle\prod_{i=1}^{n} r_{i,j}^{[k],h}\right)^{\frac{1}{n}}, &
 j=0,\ldots,J , \; k \in {\cal K}, \; h=1,\ldots,M_k \\
...
j=0,\ldots, J, \; k \in {\cal K} \\
\text{AvgRelA}^{[k]}_j & = & \left(\displaystyle\prod_{i=1}^{n} \prod_{h=1}^{M_k} r_{i,j}^{[k],h}\right)^{\frac{1}{n M_k}}, &
\; j=0,\ldots,J, \; k \in {\cal K} \\
\text{AvgRelA}^{[k]}_{a,j} & = &\left(\displaystyle\prod_{i=1}^{n_a} \prod_{h=1}^{M_k} r_{i,j}^{[k],h}\right)^{\frac{1}{n_a M_k}}, &
j=0,\ldots, J, \; k \in {\cal K} \\
\text{AvgRelA}^{[k]}_{b,j} & = &\left(\displaystyle\prod_{i=n_a+1}^{n} \prod_{h=1}^{M_k} r_{i,j}^{[k],h}\right)^{\frac{1}{n_b M_k}}, &
 j=0,\ldots, J, \; k \in {\cal K}
\end{array}
$$
...
$$
\begin{array}{rcll}
\text{AvgRelA}_{i,j} & = & \left(\displaystyle\prod_{k \in {\cal K}} \prod_{h=1}^{M_k} \text{RelA}_{i,j}^{[k],h}\right)^{\frac{1}{k^*+m}}, &
\begin{array}{l}
i=1,\ldots,n  \\ j=0,\ldots,J
\end{array} \\
\text{AvgRelA}_j & = & \left(\displaystyle\prod_{i=1}^{n} \prod_{k \in {\cal K}} \prod_{h=1}^{M_k} \text{RelA}_{i,j}^{[k],h}\right)^{\frac{1}{n(k^*+m)}}  , & j=0,\ldots,J \\
\text{AvgRelA}_{a,j} & = & \left(\displaystyle\prod_{i=1}^{n_a} \prod_{k \in {\cal K}} \prod_{h=1}^{M_k} \text{RelA}_{i,j}^{[k],h}\right)^{\frac{1}{n_a(k^*+m)}}  , & j=0,\ldots,J \\
\text{AvgRelA}_{b,j} & = & \left(\displaystyle\prod_{i=n_a+1}^{n} \prod_{k \in {\cal K}} \prod_{h=1}^{M_k} \text{RelA}_{i,j}^{[k],h}\right)^{\frac{1}{n_b(k^*+m)}} , & j=0,\ldots,J
\end{array}
$$
...
## References
Di Fonzo, T., Girolimetto, D. (2020), *Cross-Temporal Forecast Reconciliation: Optimal Combination Method and Heuristic Alternatives*, Department of Statistical Sciences, University of Padua, [arXiv:2006.08570](https://arxiv.org/abs/2006.08570)^[Appendix 7: Average relative accuracy indices for selected groups of variables/time frequencies/forecast horizons, in a rolling forecast experiment].
